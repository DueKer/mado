...
async function syncTaskToDB(task: Task) {
  try {
    await fetch(apiUrl('/api/db/tasks'), {
      method: 'POST',
...
async function deleteTaskInDB(taskId: string) {
  try {
    await fetch(apiUrl(`/api/db/tasks?id=${encodeURIComponent(taskId)}`), { method: 'DELETE' });
  } catch {}
}

async function syncDocToDB(doc: RagDocument) {
  try {
    await fetch(apiUrl('/api/db/rag'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function deleteDocFromDB(docId: string) {
  try {
    await fetch(apiUrl(`/api/db/rag?id=${encodeURIComponent(docId)}`), { method: 'DELETE' });
  } catch {}
}
...
export function useKnowledgeBase() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(apiUrl('/api/db/rag'));
        if (res.ok) {
...
export function useHistory() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(apiUrl('/api/db/tasks?limit=200'));
        if (res.ok) {
