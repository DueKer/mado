...
function HomeContent() {
  ...
  const pendingApproval = state.activeTaskId ? state.tasksById[state.activeTaskId]?.pendingApproval : null;
  const approvalRequest = pendingApproval
    ? { call: { id: pendingApproval.callId, name: pendingApproval.name, arguments: pendingApproval.arguments } }
    : null;
  const handleApprovalApprove = () => approveToolCall(true, state.activeTaskId ?? undefined);
  const handleApprovalReject = () => approveToolCall(false, state.activeTaskId ?? undefined);
  const handleApprovalApproveAll = () => approveAllToolCalls(state.activeTaskId ?? undefined);
